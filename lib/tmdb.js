...
export async function getMovieDetailsById(id) {
    ...
    const res = await fetch(
        ...
        { headers, next: { revalidate: 3600 } }, // کش یک ساعته
    );
...
export async function getLatestMoviesByGenre(
    genreId,
    limit = 20,
    sort = "desc",
) {
    ...
    const res = await fetch(
        `https://api.themoviedb.org/3/discover/movie?with_genres=${genreId}&sort_by=primary_release_date.${sort}&primary_release_date.lte=${today}&include_adult=false&language=en-US&page=1`,
        { headers },
    );
    const data = await res.json();
